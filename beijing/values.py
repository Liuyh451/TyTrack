...
class GlobalScaler:
    ...
    def get_tensors(self, t):
        ...
        min_vals = torch.tensor(self.stats[t]['min'], dtype=torch.float32, device=self.device)
        max_vals = torch.tensor(self.stats[t]['max'], dtype=torch.float32, device=self.device)
    ...
    def _get_time_offset(self, current_timestamp):
        ...
        dt_fake = dt_current.replace(year=self.base_year, month=target_month, day=target_day)
            ...
            for c in range(6):
                ...
                if channel_out.any():
                    ...
                    if above_one.numel() > 0:
                        print(f"⚠️  [Area: {self.area}] 通道 {c} 有 {above_one.numel()} 个值 > 1.0, 最大值: {above_one.max().item():.4f}")
                    if below_zero.numel() > 0:
                        print(f"⚠️  [Area: {self.area}] 通道 {c} 有 {below_zero.numel()} 个值 < 0.0, 最小值: {below_zero.min().item():.4f}")
                    print(f"    通道 {c} 整体范围: [{min_val:.4f}, {max_val:.4f}]")
                    print(f"    对应 min_vals[{c}]={min_vals[c].item():.4f}, max_vals[{c}]={max_vals[c].item():.4f}")
