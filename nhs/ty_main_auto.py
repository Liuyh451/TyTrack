...
def fetch_real_track(ty_code, report_dt):
    report_key = report_dt.strftime("%Y%m%d%H")
    output_path = (
        SCRIPT_DIR
        / "output"
        / str(ty_code)
        / f"real_track_{ty_code}_{report_key}.txt"
    )
    if output_path.exists():
        LOGGER.info(
            "real track exists, skip fetch: typhoon=%s report_time=%s",
            ty_code,
            report_key,
        )
        return
    ...
    try:
        payload = REAL_TRACK.load_typhoon_json(str(ty_code))
        points = REAL_TRACK.extract_real_track(payload, report_key)
        REAL_TRACK.save_track_txt(points, output_path)
        LOGGER.info(
            "real track saved: typhoon=%s report_time=%s points=%s path=%s",
            ty_code,
            report_key,
            len(points),
            output_path,
...
def process_one_typhoon(typhoon, report_dt):
    ...
    fetch_real_track(ty_code, report_dt)
