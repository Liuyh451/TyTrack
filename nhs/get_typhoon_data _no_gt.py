...
def parse_args():
    ...
    parser.add_argument(
        "report_time",
        help="Beijing report time, format YYYYMMDDHH, e.g. 2026062214."
    )

    parser.add_argument(
        "json_url",
        help="Typhoon detail JSON URL."
    )
    ...
    try:
        ...
        raise SystemExit(
            "report_time must use YYYYMMDDHH, e.g. 2026062214"
        ) from exc
    ...
    for m_idx, inst in enumerate(orglist):
        ...
        if not data_list:
            ...
            pre = (
                ...
                if 'pre_time' in df else np.zeros(max_steps)
            )

            lat = (
                df['lat_model'].fillna(0).values
                if 'lat_model' in df else np.zeros(max_steps)
            )

            lng = (
                df['lng_model'].fillna(0).values
                if 'lng_model' in df else np.zeros(max_steps)
            )

            mask_matrix[m_idx, :] = (
                ~df['lat_model'].isna().values
                if 'lat_model' in df else False
            )
...
def process_typhoon(json_data, report_dt):
    ...
    print(
        "精准匹配起报时间 (BJ): "
        f"{report_dt.strftime('%Y-%m-%d %H:%M:%S')}"
    )
    ...
    if all_x:
        ...
        np.save(
            os.path.join(SAVE_DIR, 'x_masks.npy'),
            np.stack(all_masks)
        )
